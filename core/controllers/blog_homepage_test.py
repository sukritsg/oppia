...
class BlogPostHandlerTest(test_utils.GenericTestBase):
    ...
    def test_raise_expection_if_blog_post_has_no_author(self):
        self.login(self.user_email)
        blog_post = blog_services.get_blog_post_by_author(self.blog_post.author)
        self.get_json(
            '%s/%s' % (
...
class AuthorsPageHandlerTest(test_utils.GenericTestBase):
    ...
    def test_get_authors_data_raises_if_user_created_new_account(self):
        self.login(self.user_email)
        json_response = self.get_json(
            '%s/%s' % (
                feconf.AUTHOR_SPECIFIC_BLOG_POST_PAGE_URL_PREFIX,
        ...
        blog_admin_model = (
            user_models.UserSettingsModel.get_by_id(self.blog_admin_id))
        blog_admin_model.created = True
        blog_admin_model.update_timestamps()
        blog_admin_model.update_new_username()
        blog_admin_model.put()
        self.get_json(
            '%s/%s' % (
                feconf.AUTHOR_SPECIFIC_BLOG_POST_PAGE_URL_PREFIX,
